...
extension FactsFlow {
  enum Actions: ReSwift.Action {
    ...
    case setSelectedFact(index: Int)
...
extension FactsFlow.Reducer {
  ...
  static func handleAction(action: ReSwift.Action, state: FactsFlow.State?) -> FactsFlow.State {
    ...
    case .setSelectedFact(let index):
...
extension FactsFlow {
  ...
  /// select fact for view details
  public static func selectFact(by index: Int) -> ReSwift.Store<AppState>.ActionCreator {
    return { state, store in
      return Actions.setSelectedFact(index: index)
    }
  }
